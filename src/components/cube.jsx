...
const Cube = () => {
    ...
    const makeCubes = async () => {
        var cubes = []
        function setupCSElements(elementId, n) {
            ...
            for (let j = 0; j < n; j++) {
                ...
                cubes.push({ "id": "cube_" + j, "r1": r1, "r2": r2, "r3": r3, "r4": r4, "r5": r5, "r6": r6, "color": colorpicks[rndInt], "startTime": startTime, "duration": duration })
            }
            ...
            if (frame > 3 + (thisFrameLag)) {
                ...
                for (let i in cubes) {
                    ...
                    eleObj.css({
                        ...
                        transform: "translate(" + (r1 * eWidth).toString() + "px, " + (r2 * eHeight - eHeight / 2).toString() + "px) scale(" + (r3 * 2 + .1).toString() + ") rotate3d(" + (r1 * 2 - 1).toString() + ", " + (r2 * 2 - 1).toString() + ", " + (r3 * 2 - 1).toString() + ", " + (rotations).toString() + "deg) ",
                        "transform-origin": "center"
                    });

                    eleObj.find(".cube-face").css(
                        {
                            "border-color": "rgba(0, 0, 0," + (opacityCurve / 8 * (r3 + .5)).toString() + ")",
                            "background-color": cube.color + (opacityCurve / 8 * (r3 + .5)).toString() + ")"
                        }
                    );
